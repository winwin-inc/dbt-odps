...
class LocalDate(object):
    ...
    def start_of_halfyear(self):
        half_year = 1 if self.month() <= 6 else 2
        start_month = 1 if half_year == 1 else 7
        return LocalDate(self.date.replace(month=start_month, day=1))

    def end_of_halfyear(self):
        half_year = 1 if self.month() <= 6 else 2
        end_month = 6 if half_year == 1 else 12
        return LocalDate(self.date.replace(month=end_month, day=1))

    def start_of_year(self):
        return LocalDate(self.date.replace(month=1, day=1))

    def end_of_year(self):
        return LocalDate(self.date.replace(month=12, day=1))
